<script setup>
    ...
    const onClick = async (e) => {
        document.sheet._onRollAction(e, props.context);
    };

    const disabled = computed(() => {
        //console.log("Action Roll disabled computed triggered", props.visibility, props.editMode);
    ...
    const hidden = computed(() => {
        //console.log("Action Roll hidden computed triggered", props.visibility, props.editMode);
...
<template>
    <v-btn :color="color" class="action-btn" @click="onClick" v-if="!hidden" :disabled="disabled"  :data-tooltip="game.i18n.localize('Spell.Roll')">{{game.i18n.localize('Spell.Roll')}}</v-btn>
</template>
