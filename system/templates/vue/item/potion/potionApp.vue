<script setup>
    ...
    import potionUseAction from './components/actions/potionUseAction.vue';
    ...
    function spawnDatatableWindow(e, pageName, tabName) {
        if (event.button === 1) {
            ...
            const tableName = `itemPotion${pageName}${tabName}`;
    ...
    <v-app>
        ...
        <v-navigation-drawer v-model="drawer" temporary style="background-color: #dddddd">
            ...
            <v-tabs v-model="page" direction="vertical">
                <v-tab value="potion" prepend-icon="fa-solid fa-circle-user">Potion</v-tab>
            </v-tabs>
            <template v-slot:append>
                <div class="pa-2">
        ...
        <v-main class="d-flex">
            <v-container :key="editMode" :class="pageBackground" fluid>
                <v-tabs-window v-model="page">
                    <v-tabs-window-item value="potion" data-tab="potion">
                        <v-row dense>
                            <v-number-input
                                controlVariant="stacked"
                                density="compact"
                                variant="outlined"
                                v-model="context.system.quantity"
                                name="system.quantity"
                                :disabled="isDisabled('quantity')" v-if="!isHidden('quantity')"
                            >
                            
                <template #label>
                    <span v-html="getLabel('Potion.Quantity', undefined)" />
                </template>
                            
                <template #append-inner>
                    <i-calculator v-if="editMode" :context="context" :systemPath="'system.quantity'" :primaryColor="primaryColor" :secondaryColor="secondaryColor"></i-calculator>
                </template>
                            ...
                            <potionUseAction 
                                ...
                                :color="primaryColor"
                                :editMode="editMode" 
                                :visibility="visibilityStates['use'].value">
                            </potionUseAction>
                        </v-row>
                        <v-divider class="mt-4 mb-2"></v-divider>
                        <v-tabs v-model="tab" grow always-center>
                                <v-tab value="description" prepend-icon="fa-solid fa-book">Description</v-tab>
                                <v-tab value="effects" prepend-icon="fa-solid fa-sparkles" @mousedown="spawnDatatableWindow($event, 'Potion', 'effects')">Effects</v-tab>
                        </v-tabs>
