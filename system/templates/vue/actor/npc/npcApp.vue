<script setup>
    ...
    import NPCNPCEquipmentDatatable from './components/datatables/npcNPCEquipmentDatatable.vue';
    ...
    function spawnDatatableWindow(e, pageName, tabName) {
        if (event.button === 1) {
            ...
            const tableName = `actorNPC${pageName}${tabName}`;
    ...
    <v-app>
        ...
        <v-navigation-drawer v-model="drawer" temporary style="background-color: #dddddd">
            ...
            <v-tabs v-model="page" direction="vertical">
                <v-tab value="npc" prepend-icon="fa-solid fa-circle-user">NPC</v-tab>
            </v-tabs>
            <template v-slot:append>
                <div class="pa-2">
        ...
        <v-main class="d-flex">
            <v-container :key="editMode" :class="pageBackground" fluid>
                <v-tabs-window v-model="page">
                    <v-tabs-window-item value="npc" data-tab="npc">
                        <v-row dense>
                            <v-number-input
                                ...
                                name="system.level"
                                :disabled="isDisabled('level')" v-if="!isHidden('level')" color="#FF7F50"
                            >
                            
                <template #label>
                    <span v-html="getLabel('NPC.Level', 'fa-solid fa-chart-line')" />
                </template>
                ...
                <template #append-inner>
                    <i-calculator v-if="editMode" :context="context" :systemPath="'system.level'" :primaryColor="primaryColor" :secondaryColor="secondaryColor"></i-calculator>
                            ...
                            <i-attribute 
                                ...
                                :editMode="editMode"
                                :hasMod="false" 
                                :mod="context.system.monster.mod"
                                systemPath="system.monster.value" 
                                :context="context" 
                                :min="0" 
                                :disabled="isDisabled('monster')" v-if="!isHidden('monster')"
                ...
                <template #label>
                    <span v-html="getLabel('NPC.Defense', undefined)" />
                </template>
                        ...
                        <v-tabs v-model="tab" grow always-center>
                                ...
                                <v-tab value="equipment" prepend-icon="fa-solid fa-table" @mousedown="spawnDatatableWindow($event, 'NPC', 'Equipment')">{{ game.i18n.localize('Equipment') }}</v-tab>
                                <v-tab value="effects" prepend-icon="fa-solid fa-sparkles" @mousedown="spawnDatatableWindow($event, 'NPC', 'effects')">Effects</v-tab>
                        </v-tabs>
                        <v-tabs-window v-model="tab" class="tabs-window">
                            ...
                            <v-tabs-window-item value="equipment" data-tab="equipment" data-type="equipment" class="tabs-container">
                                <NPCNPCEquipmentDatatable systemPath="system.equipment" :context="context"></NPCNPCEquipmentDatatable>
                            </v-tabs-window-item>
