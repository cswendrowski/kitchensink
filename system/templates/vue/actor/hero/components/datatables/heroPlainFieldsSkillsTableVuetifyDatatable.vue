<script setup>
    ...
    const customHeaders = [
        { title: game.i18n.localize("Skill.Ability"), key: 'system.ability', sortable: true },
        { title: game.i18n.localize("Skill.Trained"), key: 'system.trained', sortable: true },
        { title: game.i18n.localize("Skill.SkillMod"), key: 'system.skillmod', sortable: true },
        { title: game.i18n.localize("Skill.UsesLeft"), key: 'system.usesleft', sortable: false },
        { title: game.i18n.localize("Skill.Die"), key: 'system.die', sortable: true },
        { title: game.i18n.localize("Skill.Template"), key: 'system.template', sortable: false },
        { title: game.i18n.localize("Skill.Skill"), key: 'system.skill', sortable: true },
    ];
    ...
    const initializeColumnSettings = async () => {
        try {
            ...
            const tableSettings = documentTables['PlainFieldsSkillsTable'] || {};
    ...
    const saveColumnSettings = async () => {
        try {
            ...
            savedSettings[document._id]['PlainFieldsSkillsTable'] = tableSettings;
...
<template>
    <v-card flat class="isdl-datatable">
        <v-card-title class="d-flex align-center pe-1" style="height: 40px;">
            ...
            &nbsp; {{ game.i18n.localize("Hero.SkillsTable") }}
            <v-spacer></v-spacer>
            <v-text-field
                    v-model="search"
                    density="compact"
        ...
        >
            ...
            <!-- Custom field slots -->
            <template v-if="isColumnVisible('system.ability')" v-slot:item.system.ability="{ item }">
                <span class="text-caption">{{ humanize(getNestedValue(item, 'system.ability')) }}</span>
            </template>
            <template v-if="isColumnVisible('system.trained')" v-slot:item.system.trained="{ item }">
                <v-chip
                    :color="getNestedValue(item, 'system.trained') ? props.primaryColor : props.secondaryColor"
                    size="x-small"
                    variant="elevated"
                    class="text-caption"
                >
                    <v-icon v-if="getNestedValue(item, 'system.trained')">fa-solid fa-check</v-icon>
                    <v-icon v-else>fa-solid fa-times</v-icon>
                </v-chip>
            </template>
            <template v-if="isColumnVisible('system.usesleft')" v-slot:item.system.usesleft="{ item }">
                <v-chip 
                    label
                    :color="getResourceColor(getNestedValue(item, 'system.usesleft'))"
                    size="x-small"
                    variant="elevated"
                    class="text-caption"
                >
                    {{ getNestedValue(item, 'system.usesleft.value') }}/{{ getNestedValue(item, 'system.usesleft.max') }}
                </v-chip>
            </template>
            <template v-if="isColumnVisible('system.die')" v-slot:item.system.die="{ item }">
                <v-chip 
                    label
                    color="primary"
                    size="x-small"
                    ...
                    class="text-caption"
                >
                    {{ getNestedValue(item, 'system.die') }}
                </v-chip>
            </template>
            <template v-if="isColumnVisible('system.template')" v-slot:item.system.template="{ item }">
                <v-chip 
                    color="secondary"
                    size="x-small"
                    variant="elevated"
                    ...
                    label
                >
                    {{ getNestedValue(item, 'system.template.summary') }}
                </v-chip>
            </template>
            ...
            <template v-slot:item.actions="{ item }">
                <div class="d-flex align-center justify-center ga-1">
                    <v-tooltip :text="game.i18n.localize('Skill.Roll')">
                    ...
                    <v-tooltip :text="game.i18n.localize('Skill.SimpleFightRoll')">
                        <template v-slot:activator="{ props }">
                            <v-btn
                                v-bind="props"
                                icon="fa-solid fa-bolt"
                                ...
                                variant="text"
                                color="primary"
                                @click="customItemAction(item, 'simplefightroll')"
                            ></v-btn>
                        </template>
                    </v-tooltip>
                    <v-tooltip text="Edit">
            ...
            <template v-slot:no-data>
                <div class="text-center pa-4">
                    ...
                    <div class="text-body-2 text-medium-emphasis">
                        Add your first {{ game.i18n.localize("Skill").toLowerCase() }} to get started
                    </div>
                </div>
            </template>
        </v-data-table>
