<script setup>
    ...
    const onClick = async (e) => {
        document.sheet._onHealAction(e, props.context);
    };

    const disabled = computed(() => {
        //console.log("Action Heal disabled computed triggered", props.visibility, props.editMode);
    ...
    const hidden = computed(() => {
        //console.log("Action Heal hidden computed triggered", props.visibility, props.editMode);
...
<template>
    <v-btn :color="color" class="action-btn" @click="onClick" v-if="!hidden" :disabled="disabled"  :data-tooltip="game.i18n.localize('Hero.Heal')">{{game.i18n.localize('Hero.Heal')}}</v-btn>
</template>
