<script setup>
    ...
    const onClick = async (e) => {
        document.sheet._onLevelUpAction(e, props.context);
    };

    const disabled = computed(() => {
        //console.log("Action LevelUp disabled computed triggered", props.visibility, props.editMode);
    ...
    const hidden = computed(() => {
        //console.log("Action LevelUp hidden computed triggered", props.visibility, props.editMode);
...
<template>
    <v-btn :color="color" class="action-btn" @click="onClick" v-if="!hidden" :disabled="disabled"  :data-tooltip="game.i18n.localize('Hero.LevelUp')">{{game.i18n.localize('Hero.LevelUp')}}</v-btn>
</template>
