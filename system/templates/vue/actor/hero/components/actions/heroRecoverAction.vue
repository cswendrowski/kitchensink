<script setup>
    ...
    const onClick = async (e) => {
        document.sheet._onRecoverAction(e, props.context);
    };

    const disabled = computed(() => {
        //console.log("Action Recover disabled computed triggered", props.visibility, props.editMode);
    ...
    const hidden = computed(() => {
        //console.log("Action Recover hidden computed triggered", props.visibility, props.editMode);
...
<template>
    <v-btn :color="color" class="action-btn" @click="onClick" v-if="!hidden" :disabled="disabled"  :data-tooltip="game.i18n.localize('Hero.Recover')">{{game.i18n.localize('Hero.Recover')}}</v-btn>
</template>
