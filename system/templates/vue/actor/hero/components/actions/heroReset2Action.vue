<script setup>
    ...
    const onClick = async (e) => {
        document.sheet._onReset2Action(e, props.context);
    };

    const disabled = computed(() => {
        //console.log("Action Reset2 disabled computed triggered", props.visibility, props.editMode);
    ...
    const hidden = computed(() => {
        //console.log("Action Reset2 hidden computed triggered", props.visibility, props.editMode);
...
<template>
    <v-btn :color="color" class="action-btn" @click="onClick" v-if="!hidden" :disabled="disabled"  :data-tooltip="game.i18n.localize('Hero.Reset2')">{{game.i18n.localize('Hero.Reset2')}}</v-btn>
</template>
