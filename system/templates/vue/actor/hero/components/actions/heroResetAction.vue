<script setup>
    ...
    const onClick = async (e) => {
        document.sheet._onResetAction(e, props.context);
    };

    const disabled = computed(() => {
        //console.log("Action Reset disabled computed triggered", props.visibility, props.editMode);
    ...
    const hidden = computed(() => {
        //console.log("Action Reset hidden computed triggered", props.visibility, props.editMode);
...
<template>
    <v-btn :color="color" class="action-btn" @click="onClick" v-if="!hidden" :disabled="disabled"  :data-tooltip="game.i18n.localize('Hero.Reset')">{{game.i18n.localize('Hero.Reset')}}</v-btn>
</template>
